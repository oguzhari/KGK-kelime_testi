...
if st.button('Analiz Et'):
    if ogr_no == "" or ogr_ad == "" or ogr_mail == "":
        ...
        max_score_text = f"{max_score_label} (%{max_score/sum(sizes) * 100})"
        ...
        # texti en alta yaz
        ax.text(0, -1.5, max_score_text, color='black', fontsize=12, ha='center')
        with st.empty():
            plt.savefig(file_name, dpi=400)
            st.success("Analiz oluşturuldu, kaydediliyor...")
            send_mail.send_analysis(ogr_mail, [file_name])
            send_mail.send_analysi_to_analysis("cansusen@sakarya.edu.tr", [file_name])
